...
if __name__ == '__main__':
    ...
    img = cv2.imread(opt.ori_train_files[0])
    gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    sift = cv2.SIFT_create()
    
    kp, des = sift.detectAndCompute(gray,None)
    # img_keys=cv2.drawKeypoints(gray,kp,img,flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    # cv2.imwrite('Data Preprocessing/sift_keypoints.jpg',img_keys)
    ...
    # cv2.imwrite('Data Preprocessing/resized_sift_keypoints.jpg',resized_img_keys)
    ...
    # cv2.imwrite('Data Preprocessing/scaledmatch_sift.jpg',scaled_match)
